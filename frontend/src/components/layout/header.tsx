...
import { Activity, LogOut, Menu, LayoutDashboard, MessageSquare, Database, UserCircle, FolderOpen } from "lucide-react";
...
const adminNavItems = [
  ...
  { name: "Documents", href: ROUTES.DOCUMENTS, icon: FolderOpen, adminOnly: false },
              ...
              <Button variant="ghost" size="sm" asChild className="h-10 px-2 sm:px-3">
                <Link href={ROUTES.PROFILE} className="flex items-center gap-2">
                  <Avatar className="h-6 w-6">
                    {user?.avatar_url && (
                      <AvatarImage src={`/api/v1/users/avatar/${user.id}`} alt={user.email} />
                    )}
