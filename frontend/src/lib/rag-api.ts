...
export const RAG_API_ROUTES = {
  ...
  COLLECTIONS_DOCUMENTS: (name: string) => `/rag/collections/${name}/documents`,
  COLLECTIONS_DOCUMENT_DELETE: (name: string, documentId: string) =>
    `/rag/collections/${name}/documents/${documentId}`,
  COLLECTIONS_INGEST: (name: string) => `/rag/collections/${name}/ingest`,
  SEARCH: "/rag/search",
} as const;
...
export async function ingestFile(
  ...
  const url = `/api/v1${RAG_API_ROUTES.COLLECTIONS_INGEST(collectionName)}${replace ? "?replace=true" : ""}`;
...
export async function listSyncSources(): Promise<SyncSourceList> {
  return apiClient.get<SyncSourceList>("/rag/sync/sources");
}
