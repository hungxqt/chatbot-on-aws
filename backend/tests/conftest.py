...
# Wrap CollectorRegistry.register to safely handle duplicates during test execution
# (Starlette rebuilds middleware stacks on dependency overrides which re-registers metrics)
...
def _safe_register(self, collector):
    try:
        ...
        if "Duplicated timeseries" in str(e):
            ...
            for name in list(self._names_to_collectors.keys()):
                if name in names:
                    try:
                        self.unregister(self._names_to_collectors[name])
