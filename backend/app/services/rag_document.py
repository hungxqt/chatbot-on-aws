...
class RAGDocumentService:
    ...
    async def get_download_info(self, doc_id: str) -> tuple[str, str, str]:
        """Get S3 key and metadata for downloading a document.

        Returns:
            Tuple of (s3_key, filename, mime_type).

        Raises:
            NotFoundError: If document has no S3 key.
        """
        doc = await self.get_document(doc_id)
        s3_key = doc.vector_document_id or doc.storage_path
        if not s3_key:
        ...
        return s3_key, doc.filename, mime_type
