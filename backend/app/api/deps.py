...
async def get_redis(request: Request) -> RedisClient | None:
    """Get Redis client from lifespan state, or None if disabled."""
    return getattr(request.state, "redis", None)
...
from app.services.shared_document import SharedDocumentService


def get_shared_document_service(db: DBSession) -> SharedDocumentService:
    """Create SharedDocumentService instance with database session."""
    return SharedDocumentService(db)


SharedDocumentSvc = Annotated[SharedDocumentService, Depends(get_shared_document_service)]
...
def get_kb_client(request: Request) -> BedrockKBClient:
    """Get Bedrock KB client from lifespan state or create new."""
    if request and hasattr(request.state, "kb_client"):
        return request.state.kb_client  # type: ignore[no-any-return]
    return BedrockKBClient.from_settings()


KBClientSvc = Annotated[BedrockKBClient, Depends(get_kb_client)]
...
def get_ingestion_service(kb_client: KBClientSvc) -> IngestionService:
    """Create IngestionService instance."""
    return IngestionService(kb_client=kb_client)
