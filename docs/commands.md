...
uv run ai_agent cmd rag-sync-s3 --collection docs

# Sync from a specific prefix (folder)
uv run ai_agent cmd rag-sync-s3 --collection docs --prefix documents/

# Sync from a specific bucket
uv run ai_agent cmd rag-sync-s3 --collection docs --bucket my-bucket
```
